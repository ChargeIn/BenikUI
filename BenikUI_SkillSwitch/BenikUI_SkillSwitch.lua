...
local BenikUI_SkillSwitch = {}

-- A "static constructor" ----------------------------------------------------------------------

local function Initialize()
    setmetatable({}, BenikUI_SkillSwitch)
    BenikUI_SkillSwitch.__index = BenikUI_SkillSwitch
    Apollo.RegisterAddon(BenikUI_SkillSwitch, true, "BenikUI_SkillSwitch")
   	BenikUI_SkillSwitch.defaultSettings = { buttonWidth = 45, buttonHeight = 22,
   									buttonYOffset = -12,
   									showTooltips = true,
   									maxShortcuts = 3,
   									debug = false,
	...
	BenikUI_SkillSwitch.settings = BenikUI_SkillSwitch.defaultSettings
end

-- Public entry points -------------------------------------------------------------------------

function BenikUI_SkillSwitch:OnLoad()
	self:WriteDebug("Loading BenikUI_SkillSwitch")
	...
	self.xmlDoc = XmlDoc.CreateFromFile("BenikUI_SkillSwitch.xml")
	self.xmlDoc:RegisterCallback("OnDocLoaded", self)

	self.swapHistory = {}
end

function BenikUI_SkillSwitch:OnSlashCommand(command, args)
	local argWords = {}
	local word = ""
...
function BenikUI_SkillSwitch:OnConfigure()
	self:OpenConfigurationWindow()
end

function BenikUI_SkillSwitch:OnSave(eLevel)
...
function BenikUI_SkillSwitch:OnRestore(eLevel, tSavedData)
	if eLevel == GameLib.CodeEnumAddonSaveLevel.Character then
		data = tSavedData or {}
...
function BenikUI_SkillSwitch:OnEnteredCombat(unitChecked, bInCombat)
...
function BenikUI_SkillSwitch:OnDocLoaded()
	Apollo.CreateTimer("SkillSwitchLoadTimer", 6, false)
end

function BenikUI_SkillSwitch:LoadUI()
...
function BenikUI_SkillSwitch:CreateFlyOutButtons()
...
function BenikUI_SkillSwitch:GetActionButtonPositions()
	...
	buttonPositions = self:GetActionButtonsCarbine("BenikUI_ActionBar")

	return buttonPositions
end

function BenikUI_SkillSwitch:GetActionButtonsCarbine(addonName)
...
function BenikUI_SkillSwitch:GetActionButtonsCandyBars()
...
function BenikUI_SkillSwitch:FindLasButton(window, skillId)
...
function BenikUI_SkillSwitch:FindLasButtons(window, result)
...
function BenikUI_SkillSwitch:IsActionBarButton(window, skillId)
...
function BenikUI_SkillSwitch:CreateAbilityButtonData(window, lasIndex, windowPositionCache, type)
...
function BenikUI_SkillSwitch:PositionFlyOutButton(flyOutButton)
...
-- function BenikUI_SkillSwitch:GetAddonWindows(addon)
-- 	local windows = {}
...
function BenikUI_SkillSwitch:RepositionFlyOutButtons()
	for _, button in pairs(self.flyOutButtons) do
		self:PositionFlyOutButton(button)
	end
end

function BenikUI_SkillSwitch:OnOpenFlyOutButtonEnter( wndHandler, wndControl, x, y )
...
function BenikUI_SkillSwitch:OnOpenFlyOutButtonExit( wndHandler, wndControl, x, y )
	self:CloseFlyOutSoon()
end

function BenikUI_SkillSwitch:PopulateAndOpenFlyOut(lasIndex, type, ownerButton)
...
function BenikUI_SkillSwitch:GetCurrentTierForLasSkill(skillId)
...
function BenikUI_SkillSwitch:CreateAbilityMenuButton(caption, skillCategory, lasIndex, prio, targetTier)
...
function BenikUI_SkillSwitch:CreateSwapButton(skill, lasIndex, prio, targetTier)
	local button = Apollo.LoadForm(self.xmlDoc, "SkillSwitchButton", self.flyOutWindow, self)
...
function BenikUI_SkillSwitch:CloseFlyOutSoon(timeOut)
...
function BenikUI_SkillSwitch:CloseFlyOutByTimer()
...
function BenikUI_SkillSwitch:CloseFlyOut()
...
function BenikUI_SkillSwitch:GetSuggestedSkillsToSwapTo(lasIndex, oldId)
...
---------------------- BenikUI_SkillSwitchButton -------------------------------

function BenikUI_SkillSwitch:OnSkillSwitchButtonEnter( wndHandler, wndControl, x, y )
...
function BenikUI_SkillSwitch:OnSkillSwitchButtonExit( wndHandler, wndControl, x, y )
	self:WriteDebug("-> OnSkillSwitchButtonExit")
	self:CloseFlyOutSoon()
	self:CloseAbilityMenuSoon()
end

function BenikUI_SkillSwitch:OnSkillSwitchButtonClick( wndHandler, wndControl, eMouseButton, nLastRelativeMouseX, nLastRelativeMouseY, bDoubleClick, bStopPropagation )
...
function BenikUI_SkillSwitch:OnSkillSwitchGenerateTooltip( wndHandler, wndControl, eToolTipType, x, y )
...
function BenikUI_SkillSwitch:OnSwapCommand(args)
...
function BenikUI_SkillSwitch:Swap(oldIndex, newSkillId)
...
function BenikUI_SkillSwitch:GetTierOfSkill(skillId)
	for _, s in pairs(AbilityBook.GetAbilitiesList()) do
		if (s.nId == skillId) then
			return s.nCurrentTier
		end
...
function BenikUI_SkillSwitch:SetTier(skillId, tierLevel)
	self:WriteDebug("Setting tier " .. tierLevel .. " for skill " .. skillId)
	AbilityBook.UpdateSpellTier(skillId, tierLevel)
end

function BenikUI_SkillSwitch:AddSwapHistory(lasIndex, oldId, newId)
...
function BenikUI_SkillSwitch:AddHistoryToList(list, oldId, newId)
	if (list == nil) then
		list = {}
	end
...
function BenikUI_SkillSwitch:RemoveSwapHistory(lasIndex, skillId)
...
function BenikUI_SkillSwitch:PopulateAbilityMenu(lasIndex, skillCategory, targetTier)
...
function BenikUI_SkillSwitch:GetAbilityMenuSkills(lasIndex, skillCategory)
	local las = ActionSetLib.GetCurrentActionSet()
...
function BenikUI_SkillSwitch:OpenAbilityMenu(ownerButton)
	local x, y = self:GetAbsolutePosition(ownerButton)
...
function BenikUI_SkillSwitch:CloseAbilityMenuSoon()
	self:WriteDebug("-> CloseAbilityMenuSoon")
	Apollo.CreateTimer("SkillSwitchCloseAbilityMenuTimer",0.2,false)
end

function BenikUI_SkillSwitch:CloseAbilityMenuByTimer()
...
function BenikUI_SkillSwitch:CloseAbilityMenu()
...
function BenikUI_SkillSwitch:GetAvailableAssaultSkills()
	return self:GetAvailableSkills(Spell.CodeEnumSpellTag.Assault)
end

function BenikUI_SkillSwitch:GetAvailableSupportSkills()
	return self:GetAvailableSkills(Spell.CodeEnumSpellTag.Support)
end

function BenikUI_SkillSwitch:GetAvailableUtilitySkills()
	return self:GetAvailableSkills(Spell.CodeEnumSpellTag.Utility)
end

function BenikUI_SkillSwitch:GetAvailablePathSkills()
	return self:GetAvailableSkills(Spell.CodeEnumSpellTag.Path)
end

function BenikUI_SkillSwitch:GetAvailableSkills(skillCategory, idsToExclude)
	result = {}
	selectedIds = {}
...
function BenikUI_SkillSwitch:WriteDebug(message)
	if (self.settings.debug) then
...
function BenikUI_SkillSwitch:PrintObject(value, key, indentation)
...
function BenikUI_SkillSwitch:GetAbsolutePosition2(window, cache)
	if (cache) then
...
function BenikUI_SkillSwitch:GetAbsolutePosition(window, cache)
...
function BenikUI_SkillSwitch:GetWindowChain(window)
	result = { }
	while window do
		result[#result+1] = window
...
function BenikUI_SkillSwitch:CoalesceTables(table1, table2)
	result = {}
...
function BenikUI_SkillSwitch:RemoveFromList(list, skillId)
	local newList = {}
	if (list ~= nil) then
...
function BenikUI_SkillSwitch:AddToTopUnique(list, newId, max)
...
function BenikUI_SkillSwitch:OpenConfigurationWindow()
...
function BenikUI_SkillSwitch:OnActivationButtonWidthSliderChanged( wndHandler, wndControl, fNewValue, fOldValue )
	self.settings.buttonWidth = fNewValue
	self:RepositionFlyOutButtons()
end

function BenikUI_SkillSwitch:OnActivationButtonVertivalOffsetSliderChanged( wndHandler, wndControl, fNewValue, fOldValue )
	self.settings.buttonYOffset = fNewValue
	self:RepositionFlyOutButtons()
end

function BenikUI_SkillSwitch:OnActivationButtonHeightSliderChanged( wndHandler, wndControl, fNewValue, fOldValue, bOkToChange )
	self.settings.buttonHeight = fNewValue
	self:RepositionFlyOutButtons()
end

function BenikUI_SkillSwitch:OnFlyOutButtonSizeSliderChanged( wndHandler, wndControl, fNewValue, fOldValue )
	self.settings.menuButtonSize = fNewValue
	self:PopulateAndOpenFlyOut(1, "skill", self.flyOutButtons[1])
	self:CloseFlyOutSoon(3)
end

function BenikUI_SkillSwitch:OnConfigurationWindowCloseButtonSignal( wndHandler, wndControl, eMouseButton )
	self.configurationWindow:Show(false)
	self.configurationWindow:Destroy()
end

function BenikUI_SkillSwitch:OnEnableToolTipsClicked( wndHandler, wndControl, eMouseButton )
	self.settings.showTooltips = wndControl:IsChecked()
end

function BenikUI_SkillSwitch:OnEnablePathFlyOutClicked( wndHandler, wndControl, eMouseButton )
	self.settings.enablePathFlyOut = wndControl:IsChecked()
	self:CreateFlyOutButtons()
end

function BenikUI_SkillSwitch:OnEnableFlyoutMenuesClicked( wndHandler, wndControl, eMouseButton )
	self.settings.enableMenuButtons = wndControl:IsChecked()
end

function BenikUI_SkillSwitch:OnResetLayOut( wndHandler, wndControl, eMouseButton )
	self:CreateFlyOutButtons()
end

function BenikUI_SkillSwitch:OnNumberOfShortCutsChanged( wndHandler, wndControl, fNewValue, fOldValue )
	self.settings.maxShortcuts = fNewValue
end

function BenikUI_SkillSwitch:OnInvisibleActivationButttonsToggle( wndHandler, wndControl, eMouseButton )
