-----------------------------------------------------------------------------------------------
-- Client Lua Script for Benik_UI_MiniMap
...
local BenikUI_MiniMap = {}

function BenikUI_MiniMap:new(o)
	o = o or {}
	setmetatable(o, self)
	self.__index = self
...
function BenikUI_MiniMap:CreateOverlayObjectTypes()
...
function BenikUI_MiniMap:BuildCustomMarkerInfo()
...
function BenikUI_MiniMap:Init()
	Apollo.RegisterAddon(self)
end

function BenikUI_MiniMap:OnLoad()
	...
	self.xmlDoc = XmlDoc.CreateFromFile("BenikUI_MiniMap.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self)
end

function BenikUI_MiniMap:OnDocumentReady()
	...
	--Themen
	self.Option = Apollo.GetAddon("BenikUI")
...
function BenikUI_MiniMap:UpdateTheme()
...
function BenikUI_MiniMap:OnCharacterCreated()
	self:UpdateRapidTransportBtn()
...
function BenikUI_MiniMap:OnOptionsUpdated()
...
function BenikUI_MiniMap:OnUpdateTimer()
...
---------------------------------------------------------------------------------------------------
function BenikUI_MiniMap:OnSave(eType)
...
---------------------------------------------------------------------------------------------------
function BenikUI_MiniMap:OnRestore(eType, tSavedData)
...
function BenikUI_MiniMap:OnWindowManagementReady()
...
---------------------------------------------------------------------------------------------------
function BenikUI_MiniMap:ReloadMissions()
...
---------------------------------------------------------------------------------------------------
function BenikUI_MiniMap:OnChangeZoneName(oVar, strNewZone)
...
function BenikUI_MiniMap:UpdateZoneName(strZoneName)
...
function BenikUI_MiniMap:OnUnitPvpFlagsChanged(unitChanged)
...
function BenikUI_MiniMap:UpdatePvpFlag()
...
---------------------------------------------------------------------------------------------------
function BenikUI_MiniMap:OnMenuBtn()
...
---------------------------------------------------------------------------------------------------
function BenikUI_MiniMap:OnMenuBtnToggle(wndHandler, wndControl)
...
function BenikUI_MiniMap:OnMinusBtn()
	self.wndMiniMap:ZoomOut()
	Sound.Play(Sound.PlayUI15ZoomOutPhysical)
end

---------------------------------------------------------------------------------------------------
function BenikUI_MiniMap:OnPlusBtn()
	self.wndMiniMap:ZoomIn()
	Sound.Play(Sound.PlayUI14ZoomInPhysical)
end

function BenikUI_MiniMap:OnMapToggleBtn()
	Event_FireGenericEvent("ToggleZoneMap")
end

function BenikUI_MiniMap:OnMapGhostMode(bMode) -- Turn on/off the ghost mode notice
	self.wndMegaMapBtnOverlay:Show(bMode)
end

function BenikUI_MiniMap:OnToggleGhostModeMap() -- Turn on/off the ghost mode button (for key input toggle on and off)
	local bShow = not self.wndMegaMapBtnOverlay:IsShown()
	self.wndMegaMapBtnOverlay:Show(bShow)
end

---------------------------------------------------------------------------------------------------
function BenikUI_MiniMap:OnRotateMapCheck()
...
function BenikUI_MiniMap:OnRotateMapUncheck()
...
function BenikUI_MiniMap:OnMapPing(idUnit, tPos )
...
function BenikUI_MiniMap:OnPingTimer()
...
function BenikUI_MiniMap:OnMouseMove(wndHandler, wndControl, nX, nY)

end

function BenikUI_MiniMap:OnMapClick(wndHandler, wndControl, eButton, nX, nY, bDouble)

end

function BenikUI_MiniMap:OnMouseButtonUp(eMouseButton, nLastRelativeMouseX, nLastRelativeMouseY)

end

---------------------------------------------------------------------------------------------------
function BenikUI_MiniMap:OnFailChallenge(tChallengeData)
	self:OnRemoveChallengeIcon(tChallengeData:GetId())
end

function BenikUI_MiniMap:OnRemoveChallengeIcon(chalOwner)
...
function BenikUI_MiniMap:OnAddChallengeIcon(chalOwner, strDescription, tPosition)
...
function BenikUI_MiniMap:OnFlashChallengeIcon(chalOwner, strDescription, fDuration, tPosition)
	if self.tChallengeObjects[chalOwner] ~= nil then
		self.wndMiniMap:RemoveObject(self.tChallengeObjects[chalOwner])
	end
...
function BenikUI_MiniMap:OnStopChallengeFlashIcon()
...
function BenikUI_MiniMap:OnPlayerPathMissionActivate(pmActivated)
...
function BenikUI_MiniMap:OnPlayerPathMissionDeactivate(pmDeactivated)
	self.wndMiniMap:RemoveObjectsByUserData(GameLib.CodeEnumMapOverlayType.PathObjective, pmDeactivated)
end

---------------------------------------------------------------------------------------------------

function BenikUI_MiniMap:ReloadPublicEvents()
...
function BenikUI_MiniMap:OnPublicEventUpdate(peUpdated)
...
function BenikUI_MiniMap:OnPublicEventEnd(peEnding)
...
function BenikUI_MiniMap:OnPublicEventStart(peStarting)
...
function BenikUI_MiniMap:OnPublicEventObjectiveUpdate(peoUpdated)
...
function BenikUI_MiniMap:OnPublicEventObjectiveEnd(peoUpdated)
	self.wndMiniMap:RemoveObjectsByUserData(self.eObjectTypePublicEvent, peoUpdated)
end

---------------------------------------------------------------------------------------------------
function BenikUI_MiniMap:OnNavPointCleared()
...
function BenikUI_MiniMap:OnNavPointSet(tLoc)
...
---------------------------------------------------------------------------------------------------
function BenikUI_MiniMap:OnCityDirectionMarked(tLocInfo)
	if not self.wndMiniMap or not self.wndMiniMap:IsValid() then
		return
	end
...
function BenikUI_MiniMap:OnZoneMap_TimeOutCityDirectionEvent()
	if not self.wndMiniMap or not self.wndMiniMap:IsValid() then
		return
	end
...
---------------------------------------------------------------------------------------------------
function BenikUI_MiniMap:OnQuestStateChanged()
...
function BenikUI_MiniMap:OnOneSecTimer()
...
function BenikUI_MiniMap:OnUnitCreated(unitNew)
...
function BenikUI_MiniMap:GetDefaultUnitInfo()
	local tInfo =
	{
		strIcon = "",
		strIconEdge = "MiniMapObjectEdge",
...
function BenikUI_MiniMap:UpdateHarvestableNodes()
...
function BenikUI_MiniMap:GetOrderedMarkerInfos(tMarkerStrings)
	local tMarkerInfos = {}
...
function BenikUI_MiniMap:HandleUnitCreated(unitNew)
...
function BenikUI_MiniMap:OnHazardShowMinimapUnit(idHazard, unitHazard, bIsBeneficial)
...
function BenikUI_MiniMap:OnHazardRemoveMinimapUnit(idHazard, unitHazard)
	if unitHazard == nil then
		return
	end
...
function BenikUI_MiniMap:OnUnitChanged(unitUpdated, eType)
...
function BenikUI_MiniMap:OnUnitDestroyed(unitDestroyed)
...
function BenikUI_MiniMap:OnGroupJoin()
...
function BenikUI_MiniMap:OnGroupAdd(strName)
	for idx = 2, GroupLib.GetMemberCount() do
		local tInfo = GroupLib.GetGroupMember(idx)
...
function BenikUI_MiniMap:OnGroupRemove(strName, eReason)
...
function BenikUI_MiniMap:OnGroupLeft(eReason)
	for idx, tMember in pairs(self.tGroupMembers) do -- remove all of the group objects
		self.wndMiniMap:RemoveObject(tMember.mapObject)
	end
...
function BenikUI_MiniMap:OnGroupUpdatePosition(arMembers)
...
function BenikUI_MiniMap:DrawGroupMembers()
	for idx = 2, GroupLib.GetMemberCount() do
...
function BenikUI_MiniMap:DrawGroupMember(tMember)
...
---------------------------------------------------------------------------------------------------
function BenikUI_MiniMap:OnGenerateTooltip(wndHandler, wndControl, eType, nX, nY)
	local strTooltip = ""
...
function BenikUI_MiniMap:OnFriendshipAccountFriendsRecieved(tFriendAccountList)
	for idx, tFriend in pairs(tFriendAccountList) do
		self:OnRefreshRadar(FriendshipLib.GetUnitById(tFriend.nId))
	end
end

function BenikUI_MiniMap:OnFriendshipAdd(nFriendId)
	self:OnRefreshRadar(FriendshipLib.GetUnitById(nFriendId))
end

function BenikUI_MiniMap:OnFriendshipRemove(nFriendId)
	self:OnRefreshRadar(FriendshipLib.GetUnitById(nFriendId))
end
...
function BenikUI_MiniMap:OnFriendshipAccountFriendRemoved(nId)
	self:OnRefreshRadar()
end

function BenikUI_MiniMap:OnReputationChanged(tFaction)
	self:OnRefreshRadar()
end

function BenikUI_MiniMap:OnRefreshRadar(newUnit)
...
function BenikUI_MiniMap:OnTutorial_RequestUIAnchor(eAnchor, idTutorial, strPopupText)
	local tAnchors =
	{
...
function BenikUI_MiniMap:OnFilterOptionCheck(wndHandler, wndControl, eMouseButton)
...
function BenikUI_MiniMap:OnFilterOptionUncheck(wndHandler, wndControl, eMouseButton)
	local eCategory = wndControl:GetData()
	if eCategory == nil then
		return
	end
...
function BenikUI_MiniMap:RehideAllToggledIcons()
...
function BenikUI_MiniMap:GetToggledIconState(eSearchType)
...
function BenikUI_MiniMap:OnMatchMakerQueueBtn(wndHandler, wndControl)
...
function BenikUI_MiniMap:UpdateMatchMakerQueueBtn()
	self.wndMain:FindChild("MatchMakerQueueBtnIcon"):Show(MatchMakingLib.IsQueuedForMatching())
end

function BenikUI_MiniMap:OnRapidTransportOpen(wndHandler, wndControl)
	if wndHandler ~= wndControl then
		return
	end
...
function BenikUI_MiniMap:UpdateRapidTransportBtn()
...
-- MiniMap instance
---------------------------------------------------------------------------------------------------
local MiniMapInst = BenikUI_MiniMap:new()
MiniMapInst:Init()
