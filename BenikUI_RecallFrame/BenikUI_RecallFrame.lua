-----------------------------------------------------------------------------------------------
-- Client Lua Script for BenikUI_RecallFrame
...
-----------------------------------------------------------------------------------------------
-- BenikUI_RecallFrame Module Definition
-----------------------------------------------------------------------------------------------
local BenikUI_RecallFrame = {}
...
-- Initialization
-----------------------------------------------------------------------------------------------
function BenikUI_RecallFrame:new(o)
    o = o or {}
    setmetatable(o, self)
    self.__index = self
...
function BenikUI_RecallFrame:Init()
    Apollo.RegisterAddon(self, nil, nil, {"BenikUI_ActionBar"})
end


-----------------------------------------------------------------------------------------------
-- BenikUI_RecallFrame OnLoad
-----------------------------------------------------------------------------------------------
function BenikUI_RecallFrame:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("BenikUI_RecallFrame.xml")
...
function BenikUI_RecallFrame:OnActionBarReady()
	self.bActionBarReady = true
	self:OnDocumentReady()
end

function BenikUI_RecallFrame:OnDocumentReady()
...
function BenikUI_RecallFrame:RefreshDefaultCommand()
...
function BenikUI_RecallFrame:ResetDefaultCommand()
...
function BenikUI_RecallFrame:OnGenerateTooltip(wndControl, wndHandler, eType, spell)
...
function BenikUI_RecallFrame:GenerateBindList()
	local tRecallPoints = {}
...
function BenikUI_RecallFrame:CloseMenu()
	self.wndMenu:FindChild("Content"):DestroyChildren()
...
function BenikUI_RecallFrame:BuildRecallEntry(eContent)
...
function BenikUI_RecallFrame:OnCooldownTimer()
...
function BenikUI_RecallFrame:OnRecallBtn(wndControl, wndHandler)
...
function BenikUI_RecallFrame:OnCloseBtn()
	self:CloseMenu()
end

function BenikUI_RecallFrame:OnChangeWorld()
	self.bHaveNeighborhoods = false
	self:CloseMenu()
end

function BenikUI_RecallFrame:OnGuildResult(guildCurr, strName, nRank, eResult) -- guild object, name string, Rank, result enum
...
function BenikUI_RecallFrame:OnAbilityBookChange()
	self:CloseMenu()
	-- Process on the next frame.
	Apollo.CreateTimer("RefreshRecallTimer", 0.001, false)
end

function BenikUI_RecallFrame:OnTutorial_RequestUIAnchor(eAnchor, idTutorial, strPopupText)
	local tAnchors =
	{
...
function BenikUI_RecallFrame:OnPortButtonUp( wndHandler, wndControl, eMouseButton, nLastRelativeMouseX, nLastRelativeMouseY )
...
-----------------------------------------------------------------------------------------------
local RecallFrameInst = BenikUI_RecallFrame:new()
