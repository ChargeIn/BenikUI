-----------------------------------------------------------------------------------------------
-- Client Lua Script for BenikUI_FloatText
...
local BenikUI_FloatText = {}

local knTestingVulnerable = -1

function BenikUI_FloatText:new(o)
	o = o or {}
	setmetatable(o, self)
	self.__index = self
	return o
...
function BenikUI_FloatText:Init()
	Apollo.RegisterAddon(self)
end

function BenikUI_FloatText:OnLoad()
...
function BenikUI_FloatText:OnDocLoaded()
...
function BenikUI_FloatText:OnOptionsUpdated()
...
function BenikUI_FloatText:GetDefaultTextOption()
	local tTextOption =
	{
...
------------------------------
function BenikUI_FloatText:SetWindows()
end

function BenikUI_FloatText:SetTheme()
end
...
---------------------------------------------------------------------------------------------------
function BenikUI_FloatText:OnSpellCastFailed( eMessageType, eCastResult, unitTarget, unitSource, strMessage )
...
---------------------------------------------------------------------------------------------------
function BenikUI_FloatText:OnSubZoneChanged(idZone, strZoneName)
...
---------------------------------------------------------------------------------------------------
function BenikUI_FloatText:OnRealmBroadcastTierMedium(strMessage)
	local tTextOption = self:GetDefaultTextOption()
	tTextOption.bUseScreenPos = true
...
---------------------------------------------------------------------------------------------------
function BenikUI_FloatText:OnActionBarNonSpellShortcutAddFailed()
...
---------------------------------------------------------------------------------------------------
function BenikUI_FloatText:OnGenericError(eError, strMessage)
...
---------------------------------------------------------------------------------------------------
function BenikUI_FloatText:OnPrereqFailed(strMessage)
	self:OnGenericError(nil, strMessage)
end

---------------------------------------------------------------------------------------------------
function BenikUI_FloatText:OnGenericFloater(unitTarget, strMessage)
	-- modify the text to be shown
	local tTextOption = self:GetDefaultTextOption()
...
function BenikUI_FloatText:OnUnitEvaded(unitSource, unitTarget, eReason, strMessage)
	local tTextOption = self:GetDefaultTextOption()
...
---------------------------------------------------------------------------------------------------
function BenikUI_FloatText:OnAlertTitle(strMessage)
	local tTextOption = self:GetDefaultTextOption()
	tTextOption.fDuration = 2
...
---------------------------------------------------------------------------------------------------
function BenikUI_FloatText:OnQuestShareFloater(unitTarget, strMessage)
	local tTextOption = self:GetDefaultTextOption()
	tTextOption.fDuration = 2
	tTextOption.fFadeInDuration = 0.2
	tTextOption.fFadeOutDuration = 0.5
...
---------------------------------------------------------------------------------------------------
function BenikUI_FloatText:OnCountdownTick(strMessage)
	local tTextOption = self:GetDefaultTextOption()
...
---------------------------------------------------------------------------------------------------
function BenikUI_FloatText:OnDeath()
	local tTextOption = self:GetDefaultTextOption()
	tTextOption.fDuration = 2
...
---------------------------------------------------------------------------------------------------
function BenikUI_FloatText:OnFloaterMultiHit(tEventArgs)
...
---------------------------------------------------------------------------------------------------
function BenikUI_FloatText:OnFloaterMultiHeal(tEventArgs)
	local bCritical = tEventArgs.eCombatResult == GameLib.CodeEnumCombatResult.Critical
...
---------------------------------------------------------------------------------------------------
function BenikUI_FloatText:OnFloaterTransference(tEventArgs)
	local bCritical = tEventArgs.eCombatResult == GameLib.CodeEnumCombatResult.Critical
	if tEventArgs.unitCaster == GameLib.GetControlledUnit() then -- Target does the transference to the source
		self:OnDamageOrHealing( tEventArgs.unitCaster, tEventArgs.unitTarget, tEventArgs.eDamageType, math.abs(tEventArgs.nDamageAmount), math.abs(tEventArgs.nShield), math.abs(tEventArgs.nAbsorption), bCritical )
	else -- creature taking damage
...
---------------------------------------------------------------------------------------------------
function BenikUI_FloatText:OnCombatMomentum( eMomentumType, nCount, strText )
...
function BenikUI_FloatText:OnExperienceGained(eReason, unitTarget, strText, fDelay, nAmount)
...
function BenikUI_FloatText:OnElderPointsGained(nAmount, nRested)
	if not Apollo.GetConsoleVariable("ui.showCombatFloater") or nAmount < 0 then
		return
	end
...
function BenikUI_FloatText:OnPathExperienceGained( nAmount, strText )
...
---------------------------------------------------------------------------------------------------
function BenikUI_FloatText:OnFactionFloater(unitTarget, strMessage, nAmount, strFactionName, idFaction) -- Reputation Floater
...
---------------------------------------------------------------------------------------------------
function BenikUI_FloatText:OnChannelUpdate_Loot(eType, tEventArgs)
...
---------------------------------------------------------------------------------------------------
function BenikUI_FloatText:OnTradeSkillFloater(unitTarget, strMessage)
	if not Apollo.GetConsoleVariable("ui.showCombatFloater") then
		return
	end
...
---------------------------------------------------------------------------------------------------
function BenikUI_FloatText:OnMiss( unitCaster, unitTarget, eMissType )
	if unitTarget == nil or not Apollo.GetConsoleVariable("ui.showCombatFloater") then
		return
	end
...
---------------------------------------------------------------------------------------------------
function BenikUI_FloatText:OnDamageOrHealing( unitCaster, unitTarget, eDamageType, nDamage, nShieldDamaged, nAbsorptionAmount, bCritical )
...
------------------------------------------------------------------
function BenikUI_FloatText:OnPlayerDamageOrHealing(unitPlayer, eDamageType, nDamage, nShieldDamaged, nAbsorptionAmount, bCritical)
...
------------------------------------------------------------------
function BenikUI_FloatText:GetDefaultCCStateTextOption()
	local tTextOption = self:GetDefaultTextOption()
	tTextOption.fScale = 1.0
	tTextOption.fDuration = 2
...
------------------------------------------------------------------
function BenikUI_FloatText:ShouldDisplayCCStateFloater( tEventArgs )
	if not Apollo.GetConsoleVariable("ui.showCombatFloater") then
		return false
	end
...
------------------------------------------------------------------
function BenikUI_FloatText:OnCombatLogCCState(tEventArgs)
...
------------------------------------------------------------------
function BenikUI_FloatText:OnCombatLogImmunity(tEventArgs)
...
------------------------------------------------------------------
function BenikUI_FloatText:OnCombatLogCCStatePlayer(tEventArgs)
	if not Apollo.GetConsoleVariable("ui.showCombatFloater") then
		return
	end
...
function BenikUI_FloatText:RequestShowTextFloater( eMessageType, unitTarget, strText, tTextOption, fDelay, tContent ) -- addtn'l parameters for XP/rep
	local tParams =
	{
...
------------------------------------------------------------------
function BenikUI_FloatText:OnDelayedFloatTextTimer()
...
function BenikUI_FloatText:OnColor( wndHandler, wndControl, eMouseButton )
...
local FloatTextInst = BenikUI_FloatText:new()
FloatTextInst:Init()
