...
function FloatTextPanel:OnRealmBroadcastTierHigh(strMessage)
	...
	local wndRealmBroadcastText = wndBroadcast:FindChild("RealmBroadcastText")
	wndRealmBroadcastText:SetText('<T TextColor="UI_TextHoloBody" Font="CRB_InterfaceMedium">' .. strMessage .. '</T>')
	local nStartHeight = wndRealmBroadcastText:GetHeight()
	wndRealmBroadcastText:SetHeightToContentHeight()
	local nHeight = wndRealmBroadcastText:GetHeight()

	if nStartHeight < nHeight then
		local nLeft, nTop, nRight, nBottom = wndBroadcast:GetAnchorOffsets()
		wndBroadcast:SetAnchorOffsets(nLeft, nTop, nRight, nBottom + nHeight - nStartHeight)
	end
