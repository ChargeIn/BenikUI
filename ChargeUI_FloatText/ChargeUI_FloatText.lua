-----------------------------------------------------------------------------------------------
-- Client Lua Script for ChargeUI_FloatText
...
local ChargeUI_FloatText = {}

local knTestingVulnerable = -1

function ChargeUI_FloatText:new(o)
	o = o or {}
	setmetatable(o, self)
	self.__index = self
	return o
...
function ChargeUI_FloatText:Init()
	Apollo.RegisterAddon(self)
end

function ChargeUI_FloatText:OnLoad()
...
function ChargeUI_FloatText:OnDocLoaded()
	...
	--Register in Options
	self.Options:RegisterAddon("FloatText")
...
function ChargeUI_FloatText:OnOptionsUpdated()
...
function ChargeUI_FloatText:GetDefaultTextOption()
	local tTextOption =
	{
...
------------------------------
function ChargeUI_FloatText:SetWindows()
end

function ChargeUI_FloatText:SetTheme()
end

function ChargeUI_FloatText:LoadOptions(wnd)
...
---------------------------------------------------------------------------------------------------
function ChargeUI_FloatText:OnSpellCastFailed( eMessageType, eCastResult, unitTarget, unitSource, strMessage )
...
---------------------------------------------------------------------------------------------------
function ChargeUI_FloatText:OnSubZoneChanged(idZone, strZoneName)
...
---------------------------------------------------------------------------------------------------
function ChargeUI_FloatText:OnRealmBroadcastTierMedium(strMessage)
	local tTextOption = self:GetDefaultTextOption()
	tTextOption.bUseScreenPos = true
...
---------------------------------------------------------------------------------------------------
function ChargeUI_FloatText:OnActionBarNonSpellShortcutAddFailed()
...
---------------------------------------------------------------------------------------------------
function ChargeUI_FloatText:OnGenericError(eError, strMessage)
...
---------------------------------------------------------------------------------------------------
function ChargeUI_FloatText:OnPrereqFailed(strMessage)
	self:OnGenericError(nil, strMessage)
end

---------------------------------------------------------------------------------------------------
function ChargeUI_FloatText:OnGenericFloater(unitTarget, strMessage)
	-- modify the text to be shown
	local tTextOption = self:GetDefaultTextOption()
...
function ChargeUI_FloatText:OnUnitEvaded(unitSource, unitTarget, eReason, strMessage)
	local tTextOption = self:GetDefaultTextOption()
...
---------------------------------------------------------------------------------------------------
function ChargeUI_FloatText:OnAlertTitle(strMessage)
	local tTextOption = self:GetDefaultTextOption()
	tTextOption.fDuration = 2
...
---------------------------------------------------------------------------------------------------
function ChargeUI_FloatText:OnQuestShareFloater(unitTarget, strMessage)
	local tTextOption = self:GetDefaultTextOption()
	tTextOption.fDuration = 2
	tTextOption.fFadeInDuration = 0.2
	tTextOption.fFadeOutDuration = 0.5
...
---------------------------------------------------------------------------------------------------
function ChargeUI_FloatText:OnCountdownTick(strMessage)
	local tTextOption = self:GetDefaultTextOption()
...
---------------------------------------------------------------------------------------------------
function ChargeUI_FloatText:OnDeath()
	local tTextOption = self:GetDefaultTextOption()
	tTextOption.fDuration = 2
...
---------------------------------------------------------------------------------------------------
function ChargeUI_FloatText:OnFloaterMultiHit(tEventArgs)
...
---------------------------------------------------------------------------------------------------
function ChargeUI_FloatText:OnFloaterMultiHeal(tEventArgs)
	local bCritical = tEventArgs.eCombatResult == GameLib.CodeEnumCombatResult.Critical
...
---------------------------------------------------------------------------------------------------
function ChargeUI_FloatText:OnFloaterTransference(tEventArgs)
	local bCritical = tEventArgs.eCombatResult == GameLib.CodeEnumCombatResult.Critical
	if tEventArgs.unitCaster == GameLib.GetControlledUnit() then -- Target does the transference to the source
		self:OnDamageOrHealing( tEventArgs.unitCaster, tEventArgs.unitTarget, tEventArgs.eDamageType, math.abs(tEventArgs.nDamageAmount), math.abs(tEventArgs.nShield), math.abs(tEventArgs.nAbsorption), bCritical )
	else -- creature taking damage
...
---------------------------------------------------------------------------------------------------
function ChargeUI_FloatText:OnCombatMomentum( eMomentumType, nCount, strText )
...
function ChargeUI_FloatText:OnExperienceGained(eReason, unitTarget, strText, fDelay, nAmount)
...
function ChargeUI_FloatText:OnElderPointsGained(nAmount, nRested)
	if not Apollo.GetConsoleVariable("ui.showCombatFloater") or nAmount < 0 then
		return
	end
...
function ChargeUI_FloatText:OnPathExperienceGained( nAmount, strText )
...
---------------------------------------------------------------------------------------------------
function ChargeUI_FloatText:OnFactionFloater(unitTarget, strMessage, nAmount, strFactionName, idFaction) -- Reputation Floater
...
---------------------------------------------------------------------------------------------------
function ChargeUI_FloatText:OnChannelUpdate_Loot(eType, tEventArgs)
...
---------------------------------------------------------------------------------------------------
function ChargeUI_FloatText:OnTradeSkillFloater(unitTarget, strMessage)
	if not Apollo.GetConsoleVariable("ui.showCombatFloater") then
		return
	end
...
---------------------------------------------------------------------------------------------------
function ChargeUI_FloatText:OnMiss( unitCaster, unitTarget, eMissType )
	if unitTarget == nil or not Apollo.GetConsoleVariable("ui.showCombatFloater") then
		return
	end
...
---------------------------------------------------------------------------------------------------
function ChargeUI_FloatText:OnDamageOrHealing( unitCaster, unitTarget, eDamageType, nDamage, nShieldDamaged, nAbsorptionAmount, bCritical )
	...
	local Options = self.Options.db.profile.FloatText
	local tTextOption = self:GetDefaultTextOption()
	...
	elseif not bHeal and (unitTarget:IsInCCState( Unit.CodeEnumCCState.Vulnerability ) or eDamageType == knTestingVulnerable ) then -- vuln not crit
		...
		elseif eDamageType == GameLib.CodeEnumDamageType.HealShields then -- healing shields params
			nBaseColor = Options.HealShield
			fMaxSize = bCritical and 0.9 or 0.7

		else -- regular target damage params
			nBaseColor = Options.DMG
		end
	end
...
------------------------------------------------------------------
function ChargeUI_FloatText:OnPlayerDamageOrHealing(unitPlayer, eDamageType, nDamage, nShieldDamaged, nAbsorptionAmount, bCritical)
	...
	tTextOption.arFrames =
	{
		[1] = {fScale = fMaxSize,	fTime = 0,									nColor = nHighlightColor,	fVelocityDirection = 0,		fVelocityMagnitude = 0,},
		[2] = {fScale = fMaxSize,	fTime = 0.05,								nColor = nHighlightColor,	fVelocityDirection = 0,		fVelocityMagnitude = 0,},
...
------------------------------------------------------------------
function ChargeUI_FloatText:GetDefaultCCStateTextOption()
	local tTextOption = self:GetDefaultTextOption()
	tTextOption.fScale = 1.0
	tTextOption.fDuration = 2
...
------------------------------------------------------------------
function ChargeUI_FloatText:ShouldDisplayCCStateFloater( tEventArgs )
	if not Apollo.GetConsoleVariable("ui.showCombatFloater") then
		return false
	end
...
------------------------------------------------------------------
function ChargeUI_FloatText:OnCombatLogCCState(tEventArgs)
...
------------------------------------------------------------------
function ChargeUI_FloatText:OnCombatLogImmunity(tEventArgs)
...
------------------------------------------------------------------
function ChargeUI_FloatText:OnCombatLogCCStatePlayer(tEventArgs)
	if not Apollo.GetConsoleVariable("ui.showCombatFloater") then
		return
	end
...
function ChargeUI_FloatText:RequestShowTextFloater( eMessageType, unitTarget, strText, tTextOption, fDelay, tContent ) -- addtn'l parameters for XP/rep
	local tParams =
	{
...
------------------------------------------------------------------
function ChargeUI_FloatText:OnDelayedFloatTextTimer()
...
function ChargeUI_FloatText:OnColor( wndHandler, wndControl, eMouseButton )
