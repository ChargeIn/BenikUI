-----------------------------------------------------------------------------------------------
-- Client Lua Script for BenikUI_ActionBar
...
local BenikUI_ActionBar = {}

function BenikUI_ActionBar:new(o)
    o = o or {}
    setmetatable(o, self)
    self.__index = self
    return o
...
function BenikUI_ActionBar:Init()
    Apollo.RegisterAddon(self)
end

function BenikUI_ActionBar:OnLoad()
	self.nSelectedMount = nil
	self.nSelectedPotion = nil

	self.xmlDoc = XmlDoc.CreateFromFile("BenikUI_ActionBar.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self)
end

function BenikUI_ActionBar:OnDocumentReady()
...
function BenikUI_ActionBar:OnRestore(eType, tSavedData)
	if eType ~= GameLib.CodeEnumAddonSaveLevel.Character then
		return
	end
...
function BenikUI_ActionBar:OnPlayerEquippedItemChanged()
...
function BenikUI_ActionBar:IsWeaponEquipped()
...
function BenikUI_ActionBar:OnUnitEnteredCombat(unit)
...
function BenikUI_ActionBar:InitializeBars()
	self:RedrawStances()
	self:RedrawMounts()
	self:RedrawPotions()
...
function BenikUI_ActionBar:ArrangeWithGab(wnd,gab)
	local last = 0
...
-- Main Redraw
-----------------------------------------------------------------------------------------------
function BenikUI_ActionBar:RedrawStances()
...
function BenikUI_ActionBar:OnMountBtn(wndHandler, wndControl)
...
function BenikUI_ActionBar:RedrawPotions()
	local unitPlayer = GameLib.GetPlayerUnit()
...
function BenikUI_ActionBar:OnPotionBtn(wndHandler, wndControl)
...
function BenikUI_ActionBar:OnShowActionBarShortcut(eWhichBar, bIsVisible, nNumShortcuts)
...
function BenikUI_ActionBar:OnShowActionBarShortcutDocked(bVisible)
	self.wndArt:FindChild("BarFrameShortcut"):Show(bVisible, not bVisible)
	self:RedrawBarVisibility()
end

function BenikUI_ActionBar:ShowVehicleBar(eWhichBar, bIsVisible, nNumShortcuts)
...
function BenikUI_ActionBar:OnUpdateActionBarTooltipLocation()
	for idx = 0, 9 do
...
function BenikUI_ActionBar:HelperSetTooltipType(wnd)
	wnd:SetTooltipType(Window.TPT_OnCursor)
end

function BenikUI_ActionBar:OnTutorial_RequestUIAnchor(eAnchor, idTutorial, strPopupText)
...
function BenikUI_ActionBar:OnUpdateInventory()
	local unitPlayer = GameLib.GetPlayerUnit()
...
function BenikUI_ActionBar:OnGenerateTooltip(wndControl, wndHandler, eType, arg1, arg2)
  local xml = nil
...
function BenikUI_ActionBar:OnActionBarNonSpellShortcutAddFailed()
	--TODO: Print("You can not add that to your Limited Action Set bar.")
end

function BenikUI_ActionBar:OnCharacterCreated()
...
function BenikUI_ActionBar:OnMouseButtonInnate( wndHandler, wndControl, eMouseButton, nLastRelativeMouseX, nLastRelativeMouseY, bDoubleClick, bStopPropagation )
...
function BenikUI_ActionBar:OnMouseButtonPotion( wndHandler, wndControl, eMouseButton, nLastRelativeMouseX, nLastRelativeMouseY )
	if eMouseButton == 1 then
...
function BenikUI_ActionBar:OnMountButtonUp( wndHandler, wndControl, eMouseButton, nLastRelativeMouseX, nLastRelativeMouseY )
	if eMouseButton == 1 then
...
function BenikUI_ActionBar:OpenMountsCustomization()
